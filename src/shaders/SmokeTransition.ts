...
vec4 texCol4(vec2 p, float b)
{
    vec4 col = texture2D(uMainSampler, p, b); //b selects mip level
    return col;
}
...
//p = uv, o = random offset for per-pixel noise variation, t = time
vec4 smokeVec4(vec2 p, vec2 o, float t)
{
    const int steps = 10;
    vec4 col = vec4(0.0);
    for (int i = 1; i < steps; ++i)
    {
        //step along a random path that grows in size with time
        p += perlin(p + o) * t * 0.001 / float(i);
        p.y -= t * 0.0001; //drift upwards
        
        //sample colour at each point, using mipmaps for blur
        col += texCol4(p, float(steps-i) * t * 0.2);
    }
...
{ 
  ...
  //gl_FragColor = vec4(texture2D(uMainSampler, uv).xyz, 1.0);
  gl_FragColor = smokeVec4(uv, outTexCoord.xy, t);
